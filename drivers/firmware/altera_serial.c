 ...
 * - three free GPIOs for the signals nCONFIG, CONFIG_DONE, nSTATUS
...
struct fpga_spi {
	...
	int nstatus_gpio; /* input GPIO to read the nSTATUS line */
	int config_done_gpio; /* input GPIO to read the CONFIG_DONE line */
...
static int altera_spi_open(struct firmware_handler *fh)
{
	...
	ret = wait_on_timeout(2 * 1000,
				(gpio_get_value(this->nstatus_gpio) == 0) &&
				(gpio_get_value(this->config_done_gpio) == 0));

	if (ret != 0) {
		dev_err(dev, "FPGA does not acknowledge the programming initiation\n");
		if (gpio_get_value(this->nstatus_gpio))
			dev_err(dev, "nSTATUS is still high!\n");
		if (gpio_get_value(this->config_done_gpio))
			dev_err(dev, "CONFIG_DONE is still high!\n");
		return ret;
	}
	...
	ret = wait_on_timeout(1600 * 1000,
				gpio_get_value(this->nstatus_gpio) == 1);
	if (ret != 0) {
...
static int altera_spi_close(struct firmware_handler *fh)
{
	...
	ret = wait_on_timeout(10 * 1000,
				(gpio_get_value(this->nstatus_gpio) == 1) &&
				(gpio_get_value(this->config_done_gpio) == 1));
	if (ret == 0) {
	...
	if (gpio_get_value(this->nstatus_gpio) == 0)
		dev_err(dev, "nSTATUS is still low!\n");
	if (gpio_get_value(this->config_done_gpio) == 0)
		dev_err(dev, "CONFIG_DONE is still low!\n");
...
static int altera_spi_of(struct device_d *dev, struct fpga_spi *this)
{
	...
	name = "nstatus-gpios";
	this->nstatus_gpio = of_get_named_gpio(n, name, 0);
	if (this->nstatus_gpio < 0) {
		ret = this->nstatus_gpio;
		goto out;
	}

	name = "config-done-gpios";
	this->config_done_gpio = of_get_named_gpio(n, name, 0);
	if (this->config_done_gpio < 0) {
		ret = this->config_done_gpio;
		goto out;
	}
	...
	gpio_direction_input(this->nstatus_gpio);
	gpio_direction_input(this->config_done_gpio);
