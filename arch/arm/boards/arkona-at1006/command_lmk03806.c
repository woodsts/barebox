...
#define LMK03806_GPIO_CLK1    IMX_GPIO_NR(6, 7)
#define LMK03806_GPIO_CLK2    IMX_GPIO_NR(7, 10)
#define LMK03806_GPIO_DATA    IMX_GPIO_NR(1, 11)
#define LMK03806_GPIO_LE1     IMX_GPIO_NR(7, 1)
#define LMK03806_GPIO_LE2     IMX_GPIO_NR(7, 0)
#define LMK03806_GPIO_FTEST1  IMX_GPIO_NR(6, 18)
#define LMK03806_GPIO_FTEST2  IMX_GPIO_NR(6, 17)

/* note: both clock generators share the same DATA pin */
static const struct gpio_set at1006_clock_devs[2] = {
	{
		/* GPIO set to access the clock generator U8 */
		.le = LMK03806_GPIO_LE1,
		.clk = LMK03806_GPIO_CLK1,
		.data = LMK03806_GPIO_DATA,
		.ftest = LMK03806_GPIO_FTEST1, /* not used yet */
	}, {
		/* GPIO set to access the clock generator U11 */
		.le = LMK03806_GPIO_LE2,
		.clk = LMK03806_GPIO_CLK2,
		.data = LMK03806_GPIO_DATA,
		.ftest = LMK03806_GPIO_FTEST2, /* not used yet */
	},
};
...
static int do_lmk03806(int argc, char *argv[])
{
	...
	if (dev >= ARRAY_SIZE(at1006_clock_devs)) {
	...
	lmk03806_transmitt_list(&list[1], (size / sizeof(uint32_t)) - 2, &at1006_clock_devs[dev]);
...
BAREBOX_CMD_HELP_USAGE("lmk03806 <idx> <filename>\n")
...
BAREBOX_CMD_START(lmk03806)
	.cmd = do_lmk03806,
	.usage = "configure LMK03806 clock generator",
	BAREBOX_CMD_HELP(cmd_lmk03806_help)
BAREBOX_CMD_END
...
{
	lmk03806_init_one_bus(&at1006_clock_devs[0]);
	lmk03806_init_one_bus(&at1006_clock_devs[1]);
}
